...
impl AppConfig {
    pub fn load() -> Result<Self> {
        if let Ok(reader) = File::open("metadata.yml") {
            return serde_yaml::from_reader(reader)
                .context("Failed to parse configuration from metadata.yml");
        }

        if let Ok(reader) = File::open("/etc/config/metadata.yml") {
            return serde_yaml::from_reader(reader)
                .context("Failed to parse configuration from /etc/config/metadata.yml");
        }
